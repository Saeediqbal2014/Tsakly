  ...
  <div class="container">
    <div class="row">
      <div class="col-lg-12">
        ...
        <input type="hidden" class="current_user" value="<?= $this->session->userdata('user') ?>">
      </div>
    </div>
    <div class="row pt-4 pb-3 Project_Details">
      <div class="col-lg-12">
        <a href="<?= base_url('projects/tasks/' . $subtask[0]->project_id); ?>" class="btn user_invait_btn btn-sm float-right" style="font-size: 13px">
          + Back to Subtasks
        </a>
      </div>
          ...
          <div class="col-lg-12 card p-4 bn" style="border-top:2px solid #4fd1fe!important">
            <div class="row">
              <div class="col-lg-6"><?php $subtask = $subtask[0];
                                   if(!empty($subtask_user)){ $subtask_user = $subtask_user[0];}
                                   ?>
                <a href="<?php echo base_url('projects/view_details/'. $subtask->project_id); ?>" class="Project_Title_Color"><?= $subtask->subtask_title ?></a>
              </div>
              <div class="col-lg-6">
                <!-- Edit_Drop_Down -->
                ...
                <?php }
                  ...
                  $ddata = $this->bm->getUserName($subtask->user_id);
                ...
                } else if ($subtask->subtask_status_by == 0) {
                          
                    $this->load->model('Basic_model','bm');
                    $ddata = $this->bm->getUserName($subtask->user_id);

                     if($this->session->userdata('user') == $subtask->user_id)
                      {
                        if($subtask->subtask_priority == "complete"){
                       echo $txt = '<div class="text-dark float-right" style="font-size: 12px;font-style:italic;">Wait for approve or reject <strong><span style="color:#4fd1fe"></span></strong></div><br>';
                        }else{
                            echo "";
                        }
                         if ($this->session->userdata('user') == $subtask->user_id) {
                                if($subtask->subtask_priority == "complete"){
                                    echo $btn = '<div class="float-right"><a href="' . base_url('projects/change_subtask_status_by_admin/' . $subtask->project_subtask_id . '/' . $subtask->project_id . '/app' . '/'. $ddata[0]['name']) . '" class="text-dark" style="font-size: 12px">Approved</a> / <a href="' . base_url('projects/change_subtask_status_by_admin/' . $subtask->project_subtask_id . '/' . $subtask->project_id . '/rej' . '/'. $ddata[0]['name']) . '" class="text-dark" style="font-size: 12px">Reject</a></div>';
                                }else{
                                    echo "";
                                }
                             }
                           ...
                           echo $txt = '<div class="text-dark float-right" style="font-size: 12px;font-style:italic;">This subtask is in pending by <strong><span style="color:#4fd1fe">' . $ddata[0]['name']. '</span></strong></div><br>';
                ...
                // ////////////////////////////////////////////
                  ...
                  echo $txt = '<div class="text-dark float-right" style="font-size: 12px;font-style:italic;">This subtask has been approved by: <span style="color:#4fd1fe;"><strong>'.$ddata[0]["name"] .'</strong></span></div><br>';

                  if($this->session->userdata('user') == 9)
                  {
                    
                  $this->load->model('Basic_model','bm');
                  $ddata = $this->bm->getUserName($this->session->userdata('user'));
                  
                    echo $txt = '<div class="text-dark float-right" style="font-size: 12px;font-style:italic;">Wait for approve or reject <strong><span style="color:#4fd1fe"></span></strong></div><br>';

                    if ($this->session->userdata('user') == 9) {
                    echo $btn = '<div class="float-right"><a href="' . base_url('projects/change_subtask_status1/' . $subtask->project_subtask_id . '/' . $subtask->project_id . '/app' . '/'. $ddata[0]['name']) . '" class="text-dark" style="font-size: 12px">Approved</a> / <a href="' . base_url('projects/change_subtask_status1/' . $subtask->project_subtask_id . '/' . $subtask->project_id . '/rej' . '/'. $ddata[0]['name']) . '" class="text-dark" style="font-size: 12px">Reject</a></div>';
                ...
                else if ($subtask->subtask_status_by == 60) {

                   $this->load->model('Basic_model','bm');

                    

                   $ddata = $this->bm->getUserName(9);
                   // echo"<pre>";
                   // print_r($ddata);
                  echo $txt = '<div class="text-dark float-right" style="font-size: 12px;font-style:italic;">This subtask has been approved by Quality Controler: <span style="color:#4fd1fe;"><strong>'.$ddata[0]["name"] .'</strong></span></div>';
                  
                  $ddata = $this->bm->getUserName(1);

                  if ($this->session->userdata('user') == 1) {
                    echo $btn = '<div class="float-right"><a href="' . base_url('projects/change_subtask_status2/' . $subtask->project_subtask_id . '/' . $subtask->project_id . '/app' . '/'. $ddata[0]['name']) . '" class="text-dark" style="font-size: 12px">Approved</a> / <a href="' . base_url('projects/change_subtask_status2/' . $subtask->project_subtask_id . '/' . $subtask->project_id . '/rej' . '/'. $ddata[0]['name']) . '" class="text-dark" style="font-size: 12px">Reject</a></div>';
                ...
                else if ($subtask->subtask_status_by == 10) {

                  echo $txt = '<div class="text-dark float-right" style="font-size: 12px;font-style:italic;">This subtask has been Approved by: <span style="color:#4fd1fe;"><strong>Admin</strong></span></div>';

                }
                
                else if ($subtask->subtask_status_by == 20) {
                  echo $txt = '<div class="text-dark float-right" style="font-size: 12px;font-style:italic;">This subtask has been Rejected by: <span style="color:#4fd1fe;"><strong>Admin</strong></span></div>';

                }

                else if ($subtask->subtask_status_by == 30) {
                  echo $txt = '<div class="text-dark float-right" style="font-size: 12px;font-style:italic;">This subtask has been Rejected by: <span style="color:#4fd1fe;"><strong>'.$ddata[0]["name"] .'</strong></span></div>';


                   echo $txt = '<div class="text-dark float-right" style="font-size: 12px;font-style:italic;"> </div><br>';
                  if ($this->session->userdata('user') == 1) {
        ...
        <div class="row">
          <div class="col-lg-12 card pt-4 pb-4 bn" style="border-top:2px solid #4fd1fe!important">
            ...
            <div class="row pt-2">
              <div class="col-lg-1"></div>
              <div class="col-lg-10 text-center" style="">
                <a href="#" class="Project_Title_Color"><?= !empty($subtask_user) ? $subtask_user->name :''?></a>
                <p>
                  <span class="light_color" style="font-size: 11px"><?= !empty($subtask_user) ?  $subtask_user->email : '' ?></span>
                </p>
              </div>
              <div class="col-lg-1"></div>
            </div>
            ...
            <div class="row mt-5">
              ...
              <div class="col-lg-9 mt-4">
                  <div class="row">
                      ...
                      <div class="col-lg-12">
                          <input type="hidden" name="id" value="<?= $subtask->project_subtask_id ?>">

                          <input type="hidden" name="project_task_id" value="<?php echo $subtask->project_task_id;?>">
                        <textarea class="form-control mt-2 comment" rows="4" name="comment" placeholder="leave a comment..." required></textarea>
                      </div>
                      <div class="col-lg-12">
              ...
              <div class="col-lg-1" style="padding: 0px;margin: 0px;margin-top: 109px;">
                <button type="submit" class="btn btn-sm user_invait_btn confirm pop_up sub">Submit</button>
              </div>
            </div>
        </form>
      </div>
  ...
  $('select[name=status_subtask]').change(function() {
    ...
    $(document).on('click','.del_comment',function(){
        ...
        if(confirm('Are you sure you want to delete this comment')){
            ...
            $.ajax({
                url: '<?= site_url('projects/delete_comments/') ?>'+id,
                success: function() {
                      comments();
                }
            })
